...
Kullanım: chunk_text(text, chunk_size=10, overlap=2) -> list[str]
"""

import nltk
import re

# NLTK punkt verisini indir (eğer yoksa)
try:
    nltk.data.find('tokenizers/punkt')
except LookupError:
    nltk.download('punkt')

# Türkçe için punkt varsa onu kullanabiliriz, ama punkt genel olarak iyi çalışır.
# Daha iyi sonuç için Türkçe model eklenebilir: nltk.download('punkt_turkish')
# Ancak şimdilik varsayılan ile devam.
...
def chunk_by_sentences(sentences: list, chunk_size: int = 10, overlap: int = 2) -> list:
    ...
    Args:
        sentences (list): Cümle listesi.
        chunk_size (int): Her bir parçadaki cümle sayısı.
        overlap (int): Ardışık parçalar arasında örtüşen cümle sayısı.
    ...
    while i < len(sentences):
        ...
        # Eğer son chunk çok kısa kaldıysa ve son chunk değilse, birleştirebiliriz ama şimdilik olduğu gibi bırak.
        # İsteğe bağlı: son chunk çok kısaysa bir öncekiyle birleştirilebilir.
    
    # Son chunk'ın çok kısa olması durumunda bir öncekiyle birleştir (opsiyonel)
...
def chunk_text(text: str, chunk_size: int = 10, overlap: int = 2) -> list:
    ...
    Args:
        ...
        chunk_size (int): Parça başına cümle sayısı.
        overlap (int): Örtüşen cümle sayısı.
...
def chunk_with_word_limit(text: str, word_limit: int = 500, overlap_words: int = 50) -> list:
    ...
    Kullanmayacağız ama referans olarak ekledim.
    """
    # Basit bir kelime sayısı yaklaşımı: boşluklara göre ayır
    words = text.split()
    if not words:
        return []
